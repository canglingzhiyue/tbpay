...
public class abz implements Interpolator {
    ...
    public abz(Context context, AttributeSet attributeSet, XmlPullParser xmlPullParser) {
        this(context.getResources(), context.getTheme(), attributeSet, xmlPullParser);
    }

    public abz(Resources resources, Resources.Theme theme, AttributeSet attributeSet, XmlPullParser xmlPullParser) {
        TypedArray obtainAttributes = TypedArrayUtils.obtainAttributes(resources, theme, attributeSet, abt.STYLEABLE_PATH_INTERPOLATOR);
    ...
    private void a(Path path) {
        ...
        this.f25223a = new float[min];
        this.b = new float[min];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < min; i2++) {
            pathMeasure.getPosTan((i2 * length) / (min - 1), fArr, null);
            this.f25223a[i2] = fArr[0];
            this.b[i2] = fArr[1];
        }
        if (Math.abs(this.f25223a[0]) <= 1.0E-5d && Math.abs(this.b[0]) <= 1.0E-5d) {
            int i3 = min - 1;
            if (Math.abs(this.f25223a[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.b[i3] - 1.0f) <= 1.0E-5d) {
                int i4 = 0;
                float f = 0.0f;
                while (i < min) {
                    float[] fArr2 = this.f25223a;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        sb.append(this.f25223a[0]);
        sb.append(",");
        sb.append(this.b[0]);
        sb.append(" end:");
        int i6 = min - 1;
        sb.append(this.f25223a[i6]);
        sb.append(",");
        sb.append(this.b[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f25223a.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f25223a[i2]) {
                length = i2;
            } else {
                i = i2;
            }
        }
        float[] fArr = this.f25223a;
