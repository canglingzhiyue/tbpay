...
public class SendUpStreamTask extends TaskApiCall<PushClient, BaseVoidTask> {
    ...
    private void a(PushClient pushClient, ResponseErrorCode responseErrorCode) {
        HMSLog.i("SendUpStreamTask", "receive upstream, msgId :" + this.b + " , packageName = " + this.f7535a + " , errorCode = " + responseErrorCode.getErrorCode());
        Intent intent = new Intent("com.huawei.push.action.MESSAGING_EVENT");
        intent.setPackage(this.f7535a);
        Bundle bundle = new Bundle();
        bundle.putString(Constant.PROP_MESSAGE_ID, this.b);
        bundle.putInt("error", responseErrorCode.getErrorCode());
        bundle.putString("message_type", ErrorEnum.SUCCESS.getInternalCode() == responseErrorCode.getErrorCode() ? "sent_message" : "send_error");
    ...
    public void doExecute(PushClient pushClient, ResponseErrorCode responseErrorCode, String str, cxs<BaseVoidTask> cxsVar) {
        if (responseErrorCode.getErrorCode() == 0) {
            HMSLog.i("SendUpStreamTask", "send up stream task,Operate succeed");
            cxsVar.a((cxs<BaseVoidTask>) null);
        } else {
