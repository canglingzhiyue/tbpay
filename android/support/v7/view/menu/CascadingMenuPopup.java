...
final class CascadingMenuPopup extends MenuPopup implements j, View.OnKeyListener, PopupWindow.OnDismissListener {
    ...
    private final ViewTreeObserver.OnGlobalLayoutListener mGlobalLayoutListener = new ViewTreeObserver.OnGlobalLayoutListener() { // from class: android.support.v7.view.menu.CascadingMenuPopup.1
        ...
        public void onGlobalLayout() {
            if (!CascadingMenuPopup.this.isShowing() || CascadingMenuPopup.this.mShowingMenus.size() <= 0 || CascadingMenuPopup.this.mShowingMenus.get(0).f1360a.isModal()) {
                return;
            }
            ...
            for (a aVar : CascadingMenuPopup.this.mShowingMenus) {
                aVar.f1360a.show();
            }
        }
    };
    ...
    private final MenuItemHoverListener mMenuItemHoverListener = new MenuItemHoverListener() { // from class: android.support.v7.view.menu.CascadingMenuPopup.3
        ...
        public void onItemHoverEnter(final e eVar, final MenuItem menuItem) {
            final a aVar = null;
            ...
            while (true) {
                if (i >= size) {
                    i = -1;
                    break;
                } else if (eVar == CascadingMenuPopup.this.mShowingMenus.get(i).b) {
                    break;
                } else {
                    i++;
                }
            ...
            if (i2 < CascadingMenuPopup.this.mShowingMenus.size()) {
                aVar = CascadingMenuPopup.this.mShowingMenus.get(i2);
            }
        ...
        public void onItemHoverExit(e eVar, MenuItem menuItem) {
            CascadingMenuPopup.this.mSubMenuHoverHandler.removeCallbacksAndMessages(eVar);
        }
    };
    ...
    public static class a {
        ...
        public final MenuPopupWindow f1360a;
        public final e b;
        public final int c;

        public a(MenuPopupWindow menuPopupWindow, e eVar, int i) {
            this.f1360a = menuPopupWindow;
            this.b = eVar;
            this.c = i;
        }

        public ListView a() {
            return this.f1360a.getListView();
        }
    }
    ...
    private int findIndexOfAddedMenu(e eVar) {
        int size = this.mShowingMenus.size();
        for (int i = 0; i < size; i++) {
            if (eVar == this.mShowingMenus.get(i).b) {
                return i;
            }
        }
        return -1;
    ...
    private MenuItem findMenuItemForSubmenu(e eVar, e eVar2) {
        int size = eVar.size();
        for (int i = 0; i < size; i++) {
    ...
    private View findParentViewForSubmenu(a aVar, e eVar) {
        MenuAdapter menuAdapter;
        int i;
        int firstVisiblePosition;
        MenuItem findMenuItemForSubmenu = findMenuItemForSubmenu(aVar.b, eVar);
        ...
        while (true) {
            if (i2 >= count) {
                i2 = -1;
                break;
            } else if (findMenuItemForSubmenu == menuAdapter.mo56getItem(i2)) {
                break;
            } else {
                i2++;
            }
    ...
    private int getNextMenuPosition(int i) {
        List<a> list = this.mShowingMenus;
    ...
    private void showMenu(e eVar) {
        ...
        MenuAdapter menuAdapter = new MenuAdapter(eVar, from, this.mOverflowOnly);
        if (!isShowing() && this.mForceShowIcon) {
            menuAdapter.setForceShowIcon(true);
        } else if (isShowing()) {
            menuAdapter.setForceShowIcon(MenuPopup.shouldPreserveIconSpacing(eVar));
        }
        ...
        if (this.mShowingMenus.size() > 0) {
            ...
            view = findParentViewForSubmenu(aVar, eVar);
        } else {
            aVar = null;
            view = null;
        }
        ...
        this.mShowingMenus.add(new a(createPopupWindow, eVar, this.mLastPosition));
        createPopupWindow.show();
        ListView listView = createPopupWindow.getListView();
        listView.setOnKeyListener(this);
        if (aVar != null || !this.mShowTitle || eVar.m() == null) {
            return;
        }
        FrameLayout frameLayout = (FrameLayout) from.inflate(R.layout.abc_popup_menu_header_item_layout, (ViewGroup) listView, false);
        frameLayout.setEnabled(false);
        ((TextView) frameLayout.findViewById(16908310)).setText(eVar.m());
    ...
    public boolean isShowing() {
        return this.mShowingMenus.size() > 0 && this.mShowingMenus.get(0).f1360a.isShowing();
    }
    ...
    public void onCloseMenu(e eVar, boolean z) {
        ...
        if (i < this.mShowingMenus.size()) {
            this.mShowingMenus.get(i).b.b(false);
        }
        a remove = this.mShowingMenus.remove(findIndexOfAddedMenu);
        remove.b.b(this);
        if (this.mShouldCloseImmediately) {
            remove.f1360a.setExitTransition(null);
            remove.f1360a.setAnimationStyle(0);
        }
        remove.f1360a.dismiss();
        int size = this.mShowingMenus.size();
        this.mLastPosition = size > 0 ? this.mShowingMenus.get(size - 1).c : getInitialMenuPosition();
        if (size != 0) {
            if (!z) {
                return;
            }
            this.mShowingMenus.get(0).b.b(false);
            return;
        }
        dismiss();
    ...
    public boolean onSubMenuSelected(o oVar) {
        ...
        if (oVar.hasVisibleItems()) {
            addMenu(oVar);
            j.a aVar2 = this.mPresenterCallback;
            if (aVar2 != null) {
                aVar2.a(oVar);
            }
            return true;
    ...
    public void updateMenuView(boolean z) {
        for (a aVar : this.mShowingMenus) {
            toMenuAdapter(aVar.a().getAdapter()).notifyDataSetChanged();
        }
    }
}
