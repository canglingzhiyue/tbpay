...
public abstract class ConstraintHelper extends View {
    ...
    private void addRscID(int i) {
        ...
        int[] iArr = this.mIds;
        if (i2 > iArr.length) {
            this.mIds = Arrays.copyOf(iArr, iArr.length << 1);
        }
        int[] iArr2 = this.mIds;
        int i3 = this.mCount;
        iArr2[i3] = i;
    ...
    private void addTag(String str) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if ((layoutParams instanceof ConstraintLayout.LayoutParams) && trim.equals(((ConstraintLayout.LayoutParams) layoutParams).aA)) {
                if (childAt.getId() == -1) {
    ...
    public void loadParameters(c.a aVar, android.support.constraint.solver.widgets.f fVar, ConstraintLayout.LayoutParams layoutParams, SparseArray<ConstraintWidget> sparseArray) {
        if (aVar.c.ae != null) {
            setReferencedIds(aVar.c.ae);
        } else if (aVar.c.af != null && aVar.c.af.length() > 0) {
            aVar.c.ae = convertReferenceString(this, aVar.c.af);
        }
        fVar.K();
        if (aVar.c.ae != null) {
            for (int i = 0; i < aVar.c.ae.length; i++) {
                ConstraintWidget constraintWidget = sparseArray.get(aVar.c.ae[i]);
                if (constraintWidget != null) {
                    fVar.b(constraintWidget);
                }
            }
        }
    }
    ...
    public void updatePreLayout(ConstraintLayout constraintLayout) {
        ...
        this.mHelperWidget.a(constraintLayout.mLayoutWidget);
    }

    public void updatePreLayout(android.support.constraint.solver.widgets.e eVar, h hVar, SparseArray<ConstraintWidget> sparseArray) {
        hVar.K();
        for (int i = 0; i < this.mCount; i++) {
            hVar.b(sparseArray.get(this.mIds[i]));
        }
    }
    ...
    public void validateParams() {
        ...
        ((ConstraintLayout.LayoutParams) layoutParams).al = (ConstraintWidget) this.mHelperWidget;
    }
}
